...
const string strMessageMagic = "gamertip Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2014-04-19 Adriano https://gamertiptalk.org/index.php?action=profile;u=112568
        // The following address was lost during distribution with 196780608.602771 coins in it. Blocking just in case :-)
        static const CgamertipAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){  // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){  // extract the destination of the previous transaction's vout[n]
                CgamertipAddress addressSource(source);
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // Subsidy is cut in half every 1,576,800 blocks, which will occur approximately every week
    nSubsidy >>= (nHeight / 420);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal gamertip miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in gamertip
    bool fStrictPayToScriptHash = true; // Always active in gamertip
    ...
    // ppcoin: fees are not collected by miners as in gamertip
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "gamertip", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "24th July 2015, the day when Schools in Nairobi to Take a Break Ahead of Obama Arrival";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("42f0d313c875bf649ecb49a98df7106b287adb17d29f72fe1b8f23b168a11e30"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("gamertipMiner : proof-of-work not meeting target");

    //// debug print
    printf("gamertipMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("gamertipMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("gamertipMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadgamertipMiner(void* parg);

static bool fGenerategamertips = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void gamertipMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("gamertip-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerategamertips || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerategamertips) && !fProofOfStake)
                return;
        }
        ...
        printf("Running gamertipMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadgamertipMiner(void* parg)
{
    ...
    try
    {
        ...
        gamertipMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadgamertipMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadgamertipMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadgamertipMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generategamertips(bool fGenerate, CWallet* pwallet)
{
    fGenerategamertips = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerategamertips = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d gamertipMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadgamertipMiner, pwallet))
                printf("Error: NewThread(ThreadgamertipMiner) failed\n");
            Sleep(10);
        }
    }
}
