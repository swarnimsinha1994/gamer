...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("GTP")
    updateDisplayUnit();
}
...
{
    QString ret = QString("gamertip:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GTP unit input in lnReqAmount, we generate the URI with GTP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(gamertipUnits::format(gamertipUnits::GTP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
